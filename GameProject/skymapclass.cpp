...
bool SkymapClass::Initialize(ID3D11Device* device, WCHAR* textureFilename, int LatLines, int LongLines)
{
	...
	result = D3DCompileFromFile(L"./data/skymap.fx", 0, 0, "SKYMAP_VS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &SKYMAP_VS_Buffer, 0);
	if (FAILED(result))
	{
		return false;
	}
	result = D3DCompileFromFile(L"./data/skymap.fx", 0, 0, "SKYMAP_PS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &SKYMAP_PS_Buffer, 0);
	if (FAILED(result))
	{
		return false;
	}
